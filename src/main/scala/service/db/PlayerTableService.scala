...
object PlayerTableService {
  def apply[F[_]: Async](transactor: Resource[F, Transactor[F]]): PlayerTableService[F] =
    new PlayerTableService[F] {
      ...
      override def insert(player: Player): F[PlayerId] = transactor.use(_.transact(PlayerTable += player)).map(_ => player.id)

      override def update(player: Player): F[Unit] = transactor.use(_.transact(PlayerTable.byId(player.id).update(player))).void
